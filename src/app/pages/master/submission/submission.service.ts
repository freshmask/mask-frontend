...
export class SubmissionService {
  ...
  getClaimTravel(page: number, size: number): Observable<any> {
    return new Observable((observer: Observer<any>) => {
      this.http.get(`api/claimtravel?access_token=` + JSON.parse(window.sessionStorage.getItem('token')).access_token)
  ...
  getClaimPA(page: number, size: number): Observable<any> {
    return new Observable((observer: Observer<any>) => {
      this.http.get(`api/claimPA?access_token=` + JSON.parse(window.sessionStorage.getItem('token')).access_token)
        .pipe(map((responseData: any) => {
          const temp = {
            content: responseData.content,
            totalPages: responseData.totalPages,
  ...
  getClaimPAR(page: number, size: number): Observable<any> {
    return new Observable((observer: Observer<any>) => {
      this.http.get(`api/claimsPAR?access_token=` + JSON.parse(window.sessionStorage.getItem('token')).access_token)
        .pipe(map((responseData: any) => {
          const temp = {
            content: responseData.content,
            totalPages: responseData.totalPages,
